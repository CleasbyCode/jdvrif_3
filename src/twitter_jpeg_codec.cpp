...
struct CodecState {
    CodecState() = default;
    // Validation and allocation can throw without going through error_exit.
    // Own cleanup here as well as on the explicit libjpeg longjmp path.
    ~CodecState() noexcept;
...
CodecState::~CodecState() noexcept {
    cleanup(*this);
}
...
ImageInfo inspectJpeg(std::span<const std::uint8_t> jpeg) {
    ...
    state->source.progress = &error->source_progress.pub;
...
Bytes prepareProgressiveSourceQuality(std::span<const std::uint8_t> jpeg) {
    ...
    state->destination.progress = &error->destination_progress.pub;
