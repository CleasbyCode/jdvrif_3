...
Recovery of older **KDF2** images prints a warning: their compression flag is unauthenticated, so altering it can change how the decrypted bytes are interpreted without an authentication failure. Verify recovered legacy files against a trusted original. Newly concealed images authenticate this flag.
