...
[[nodiscard]] constexpr bool isCoefficientCarrierFormat(
    RecoveryFormat format) noexcept {

    return format == RecoveryFormat::reddit ||
        format == RecoveryFormat::twitter;
}
...
[[nodiscard]] constexpr std::size_t maxDeclaredSpan(RecoveryFormat format) noexcept {
    switch (format) {
        ...
        case RecoveryFormat::twitter:
            return TWITTER_UPLOAD_SIZE_LIMIT;
    }
    return 0;
}
...
void recoverFromCipherExtractor(
    ...
    throwIf(
        isCoefficientCarrierFormat(format) && carrier_pin == nullptr,
        "Internal Error: Coefficient-carrier recovery is missing its PIN.");
    ...
    runWithRecoverStageFiles([&](StagingFile& cipher_stage, StagingFile& stream_stage) {
        ...
        const KdfMetadataVersion metadata_version =
            isCoefficientCarrierFormat(format)
        ...
        if (metadata_version == KdfMetadataVersion::v2_secretstream) {
            // Old streams cannot authenticate this choice retrospectively.
            // Preserve recovery compatibility while making the weaker
            // guarantee visible before interpreting or publishing plaintext.
            std::println(stderr,
                "Warning: Legacy KDF2 image: the compression flag is unauthenticated "
                "and may have been altered. Verify the recovered file against a trusted original.");
            throwIf(std::fflush(stderr) != 0 || std::ferror(stderr) != 0,
                "Output Error: Failed to deliver legacy recovery warning.");
        }
...
void recoverFromCoefficientCarrierPath(
    Envelope envelope,
    SecurePin recovery_pin,
    RecoveryFormat format) {

    const std::size_t embedded_file_size = envelope.encrypted_data.size();
...
void recoverFromIccPath(
    ...
    recoverFromCipherExtractor(metadata_vec, RecoveryFormat::default_icc, is_data_compressed, embedded_file_size, nullptr, [&](const fs::path& cipher_path) {
...
void recoverFromBlueskyPath(
    ...
    recoverFromCipherExtractor(metadata_vec, RecoveryFormat::bluesky, true, embedded_file_size, nullptr, [&](const fs::path& cipher_path) {
...
void recoverFromRedditPath(RedditEncryptedEnvelope envelope, SecurePin recovery_pin) {
    recoverFromCoefficientCarrierPath(
        std::move(envelope),
