...
OutputFile::~OutputFile() noexcept {
    if (fd_ < 0) return;
    // close() is the only path that can report a write failure, and every caller
    // takes it. Reaching the destructor with bytes still buffered therefore means
    // either an error path (whose file is discarded straight after) or a caller
    // that forgot to close -- and for the latter, dropping the tail silently
    // would truncate the output with nothing to show for it. Flush best-effort;
    // a destructor has nowhere to report the failure, so swallow it here rather
    // than terminate.
    if (fill_ > 0) {
        try {
            drain(WRITE_COMPLETE_ERROR);
        } catch (...) {
        }
    }
    ::close(fd_);
...
void validateFilenameProperties(const fs::path& path, FileTypeCheck file_type) {
    ...
    if (file_type == FileTypeCheck::cover_image ||
        file_type == FileTypeCheck::reddit_cover_image ||
        file_type == FileTypeCheck::twitter_cover_image) {
...
void validateSizeAgainstType(std::size_t file_size, FileTypeCheck file_type) {
    ...
    if (file_type == FileTypeCheck::reddit_cover_image) {
        if (MINIMUM_IMAGE_SIZE > file_size) throw std::runtime_error("File Error: Invalid image file size.");
        if (file_size > REDDIT_UPLOAD_SIZE_LIMIT) {
            throw std::runtime_error(
                "Image File Size Error: Cover image exceeds Reddit's 20 MiB upload size limit.");
        }
    }
    if (file_type == FileTypeCheck::twitter_cover_image) {
        if (MINIMUM_IMAGE_SIZE > file_size) throw std::runtime_error("File Error: Invalid image file size.");
        if (file_size > TWITTER_UPLOAD_SIZE_LIMIT) {
            throw std::runtime_error(
...
vBytes readFile(const fs::path& path, FileTypeCheck file_type, std::size_t max_bytes) {
    validateFilenameProperties(path, file_type);
    requireReadableRegularFile(path);
    ...
    validateSizeAgainstType(file_size, file_type);
    throwIf(file_size > max_bytes, "Read Error: Input file exceeds the permitted size limit.");
